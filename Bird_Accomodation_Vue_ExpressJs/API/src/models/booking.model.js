...
module.exports = {
    ...
    getAllBookings: async () => {
        let con = await config.connection();
        const request = new con.Request();
        const returnData = request.query("SELECT b.*, bi.bird_name, u.address, u.name as user_name \n " +
        "FROM [Booking] b join [Bird] bi on b.bird_id = bi.bird_id join [User] u on b.user_id = u.user_id");
        con.close();
